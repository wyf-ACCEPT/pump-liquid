...
export async function deployTokens() {
  ...
  await mockBTCB.transfer(user1.address, parseEther("10"))
  await mockWBTC.transfer(user1.address, parseEther("10"))
  await mockUSDC.transfer(user1.address, parseEther("50000"))
  await mockUSDT.transfer(user1.address, parseEther("50000"))
  await mockBTCB.transfer(user2.address, parseEther("10"))
  await mockWBTC.transfer(user2.address, parseEther("10"))
  await mockUSDC.transfer(user2.address, parseEther("50000"))
  await mockUSDT.transfer(user2.address, parseEther("50000"))
